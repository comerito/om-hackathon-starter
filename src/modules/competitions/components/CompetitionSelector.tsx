    ...
    <div ref={ref} className="relative">
      <button
        ...
        className={cn(
          ...
          open ? 'border-portal-primary/30 bg-portal-primary/5 shadow-sm' : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm',
        )}
      ...
      >
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={cn('text-gray-400 transition-transform', open && 'rotate-180')}>
          <path d="m6 9 6 6 6-6" />
        </svg>
      </button>

      {open && (
        <div className="absolute right-0 top-full z-50 mt-1.5 w-64 overflow-hidden rounded-xl border border-gray-100 bg-white shadow-xl" style={{ animation: 'cssFadeIn 150ms ease-out' }}>
            ...
            {competitions.map((c) => {
              ...
              return (
                <button
                  ...
                  className={cn('flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors', active ? 'bg-portal-primary/5' : 'hover:bg-gray-50')}
                >
